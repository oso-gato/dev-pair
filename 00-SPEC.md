# The Dev Pair — CONFIRMED SPEC (objective + build principles)

> **THE WHY and THE HOW WE BUILD**, in one document — because they are confirmed together,
> once, in a single session, and share one authority. **Status: DRAFT — pending maintainer
> confirmation.** Once confirmed this document is fixed: amendment is a new maintainer
...
- **O1 · Initiation — exactly one interactive session at the very beginning.** The maintainer
  states the objective and the intended outcome, and sets the scope and the boundaries.
- **O2 · Confirmation — the dev pair confirms the objective in that chat session.** The pair
  **co-creates the confirmed objective with the maintainer from that discussion** —
  ...
  workflow requires.
- **O3 · Execution — then autonomous, to ship.** The confirmed objective is **locked as
...
- **O4 · Immutability — the throwaway principle.** The workflow operates on **throwaway builds
...
**O5** — In addition to its role in the dev pair, the host serves as a **mother platform**: it
...
**O6** — Any other clause in this document — access, security, mechanics, roles — describes **how**
...
- **O7 · The host** — a Fedora system (the latest stable Fedora release) turned by this
...
- **O8 · The dev-container** — a containerised development environment running on that host.
...
- **O9 · The agent layer — the claudebox and its lineages.** On both the host and the
...
**O10** — The two components are **one system**: one lifecycle, one specification, one repository,
...
**O11 · One repository, many pairs.** The platform is **replicable**: this repository is
...
- **O12 · Virtualization.** A VPS host is itself a virtualized environment, so full
...
- **O13 · GPU acceleration.** Certain apps and services hosted on the host require GPU
...
**O14** — The tracks differ in capability, never in contract: same specification, same loop, same
...
- **O15 · The host** — genesis (cold-start, per the provisioning class: **cloud genesis** from a
...
- **O16 · The dev-container** — **develop + validate (tier-1)** features and images; the
...
- **O17 · Immutable host, containerise-everything.** The host OS is immutable and every
  application — the dev-container, the agent tooling, all workloads — runs in a container.
- **O18 · Three decoupled clocks.** The host OS, the dev-container image, and each **agent
...
- **O19 · Multi-tenant development.** The dev-container runs an undefined number of isolated
...
- **O20 · Two-tier validation.** A session proves whatever it can **inside the dev-container** and
...
- **O21 · Merged is not live — in both directions.** A merge is not a deployment. A merge to
  ...
  target.
- **O22 · Distrust, made structural.** No outcome is accepted on testimony — not an agent's word,
...
- **O23 · Minimal machinery.** The platform builds the smallest mechanism set that advances this
...
- **O24 · The ship gate.** Nothing ships on its builder's own word. A product ships only after an
...
1. **O25 · Secure public access with minimal surface.** Every service is locked behind
...
2. **O26 · Key-based authentication, without exception on any shell path.** Every public door
...
3. **O27 · Session persistence, as an outcome.** Interactive sessions must **survive frequent
...
- **O28** — The host never merges; the dev-container merges — **except the confirmed spec**, which
  only the maintainer merges.
- **O29** — Neither component builds production images; CI builds them. The host builds only
  throwaway validation candidates.
- **O30** — No unmerged change is ever applied to either component. Proposing is never applying.
- **O31** — The platform operates on **the repositories its GitHub App is installed on** — the
  maintainer's live install choice, private repositories included — and **never widens its
  own reach**.
- **O32** — No secrets in the repository or in image layers; credentials enter at runtime only.
- **O33** — **Headless is binding.** Nothing in the platform may assume a physical display or a
...
**O34** — This objective and the build principles are **confirmed once and fixed**; amendment is a new
...
**O35 · Why objective and principles are two documents.** The objective is **scoped** — this
...
> (`00-SPEC.md` Part O) and sharing its authority: fixed thereafter, amendment is a new
...
A tool that exists **only** in a language ecosystem (npm, PyPI, crates, gems) is admitted
**project-local, never system-wide**. The language runtime itself comes from dnf at PU1's
**L1** — Fedora ships `nodejs`, `python3` and `cargo` — so only the tool's *dependencies* are
ecosystem-sourced: they install into a **self-contained tree inside the project that owns
them**, resolved offline from a **committed lockfile with per-package integrity hashes**,
invoked from that tree, and deleted with the project. **Nothing is ever installed onto PATH
on any component** — a global language-manager install is unmanaged by the system package
manager, invisible to it, can shadow a managed binary, and survives outside the
merge-and-deploy path; that is the only form of this that pollutes. Before adopting this
