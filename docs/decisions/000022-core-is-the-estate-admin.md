...
The gap surfaced against a live fact. Verification of the erebus OS record established that Hostinger's template hands over root, where upstream Fedora Cloud Base locks root and creates a `fedora` user — so the two lineages start from different delivered identities and were being reconciled by each lineage's own day-zero contract rather than by a stated rule.

## Decision

Maintainer-instructed. On every host the estate operates, whatever its lineage or image, the first administrative user outside root is `core`. Root never authenticates remotely after genesis, and escalation is by sudo. A provider that hands over root at delivery is a day-zero input, not an exception — the VPS lineage's pasted script creates `core` and retires root, and the bare-metal lineage bakes `core` from the trust root at image build, and both arrive at the same identity by different paths.
