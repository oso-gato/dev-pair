...
- Mechanical gates live (ticket #7): the CI merge gate and the P2/P4/P6 scans, one home in `shared/gates/`, every scanner fixture-proven red before trusted green; branch protection binds the gate to the exact head sha on `main`. First component code since the reset; pre-loop bootstrap epoch — authored by the genesis agent, merged after non-author adversarial review. The review (P10 working as designed) caught two blocking defects before merge, both fixed and pinned by regression proof: the channel scan let `curl | sudo bash`, `tar --directory=/usr`, and `npm --location=global` past channels it claimed to cover; and the workflow interpolated the untrusted PR branch name into a shell (Actions script-injection), now routed through `env`. A second review pass closed two non-blocking hardenings in the same ticket: the content scans now exclude only `fixtures/` (not the whole test tree, which was a disclosed laundering path — the harness sources its violation payloads from the fixtures so it carries none itself), and the curl-pipe pattern is anchored to the command after the pipe so benign pipelines (`| tee x.sh`, `| ssh host`) no longer false-positive.
