...
- **The host** — genesis per its lineage's deploy mechanism (bylaw-instantiated); operate and maintain the platform: deploy, refresh, roll back, create and remove containers, keep itself sound; host applications per the second objective, each its own container from a registry, a VM only as P7 allows; live-diagnose and develop fixes — **PRs only, never merges**; never builds production images (CI does); never applies an unmerged change. On merge it renews itself, no standing human tap (R4).
- **The dev-container** — develop and validate (tier-1); the platform's **sole merge authority** under its empirical gates — except the `00-` artifacts and constitution, maintainer-merge-only; home of the multi-tenant sessions; refreshed from outside, by the host.
- **The agent layer** — every agent in a disposable box per universal P8, so the components stay relatively stable while every agent stays always current, and no rebuild costs live work.
...
**Two capability tracks.** Pairs differ in capability, never in contract. The **bare-metal track** carries full virtualization (libvirt/KVM) and a GPU as shared substrate — shared to containers and VMs, never monopolised from the host. The **VPS track** is assumed to have neither. Validation needing those capabilities routes to bare metal (P9); a capability may be used where it exists, but no platform function may require it.
...
- **Three decoupled clocks.** The host OS, the dev-container image, and each agent box advance on their own cadence — the boxes' the fast, nightly one — none forcing a rebuild of the others, each advance verified by reading the live artifact back.
...
- **Merged is not live — in both directions.** A merge is not a deployment: it self-arms the matching renewal (R4), staggered so neither component rebuilds the agent doing the work, and every refresh is confirmed by reading the live artifact back against merged source, fail-closed.
...
2. **Keys, never passwords.** Every public door authenticates by SSH keys only; no password ever authenticates remote shell access anywhere in the platform, public or private. The maintainer's published key set is the single trust root (P6): access is granted and revoked by managing keys, never by distributing credentials. Private-network consoles may use their own hardened per-service authentication — never a shared or default credential. Root never authenticates remotely after genesis: the administrative user is `core`, in the CoreOS convention, escalating by sudo password — a password that exists for escalation only and never authenticates a remote shell.
...
- The host never merges; the dev-container merges — except the confirmed `00-` artifacts and constitution, which only the maintainer merges.
...
- No secrets in the repository or in image layers; credentials enter at runtime only (P6).
