...
>
> **Identifiers.** **O<n>** objective clauses · **PU<n>** universal build principles (bind
> every project the pair develops) · **PR<n>** repo-specific build principles (bind this
> repository's own platform).
...
**O35 · Why the objective is scoped and the principles are universal.** The objective is **scoped** — this
...
PU1=BP1 (strengthened: one fetch contract; repo-metadata signatures) · PU2=BP2 (strengthened:
environment/release re-verification incl. virtualization + GPU availability; adoption
trail) · PU3=BP3 (extended: gates-are-features, activation-proof, no-standing-red — the
zero-base minimalism doctrine) · PR1=BP4 (amended: two sanctioned mechanisms per
provisioning class — cloud converger, local bootc image; environment-adapter genesis;
VM-as-recorded-capability) · PR2=BP5 (amended and renamed to the objective's throwaway
principle: cache invariant; bounded input registry) · PU4=BP6 (strengthened: harness sites
named) · PR3=BP7 (rescoped: atomic contracts in one repo + runtime stagger; versioning made
standing) · PU5=BP8 (extended: the suite gates; production-only-line rule; the bare-metal capability
track for full-virtualization/GPU validation) · PU6=BP9 (extended: vendoring
preferred over parity checkers; memoir-is-not-specification) · PU7=new (codified from the
measured gate-without-recovery incidents) · PR4=new (the agent-layer discipline —
